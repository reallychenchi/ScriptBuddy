...
*   **支持角色别名**：支持用户自定义角色名称（如将“甲”映射为“刘涛”）。用户回答时可以说“我是刘涛”，App自动匹配到对应角色。
*   **App提问**：“请通过语音告诉我，您想扮演‘甲’还是‘乙’？”（或者使用自定义的角色名字）。
*   **用户回答**：（如“我想演甲”、“我是乙”、“我是刘涛”）。
*   **处理**：录音 -> STT转文字 -> LLM语义分析 -> 确定角色 -> 开始对戏。

#### 2. 台词对练 & 翻页逻辑
*   **陪练念**：播放TTS，动画展示“说话中”，播完自动触发用户回合。**注意：陪练说话时必须强制关闭语音识别（互斥），避免录入App自己的声音。**
*   **用户念**：动画展示“聆听中”，进行语音识别。**注意：用户说话时App保持静音。**识别出来文字以后，完成一句话，就提交大模型，判断用户是否有：1、停止；2、重新开始；3、改变难度级别；4、角色选择 三种意图，如果存在三种意图，中断台词对练，来到对应流程。
*   **过关判定**：
...
### 关键配置管理
所有敏感配置（API Keys, Token）**必须**通过后端 API 下发，严禁硬编码在前端代码或 .env 文件中。前端启动时需调用 `GET /api/config` 获取。
